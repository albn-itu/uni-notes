...
\subsection{Part a}
\question{
	Compute the probability that $2 \leq X \leq 6$.
}
...
\subsection{Part b}
\question{
	Let $Y$ be another exponentially distributed random variable, which has parameter $\lambda = \frac{1}{4}$ , and suppose $Cov(X, Y) = -3$.\\

	Compute the mean and variance of $X + Y$
}
